...
def get_abnormalities_detection(file: UploadFile = File(...), session: Session = Depends(get_session)):
    image, image_id, file_path = save_image(file)
    image = np.array(image)
    image = cv2.cvtColor(image, cv2.COLOR_GRAY2RGB)
    annotated_image, bbox, confidence, inference_time = abnormality_detector.inference_with_annot_image(image)
    annotated_image = Image.fromarray(annotated_image.astype("uint8")).convert("RGB")
    output_image = BytesIO()
    annotated_image.save(output_image, "PNG")
    output_image.seek(0)
    
    inf_log = AbnormalityDetectionLog(image_id=image_id, file=str(file_path), inference_time=inference_time, predicted_bbox=bbox, confidence=confidence)
    session.add(inf_log)
    session.commit()
    session.refresh(inf_log)
    
    return StreamingResponse(output_image, media_type="image/png", headers={"image_id":image_id})


@r.post("/covid-condition")
def get_condition_classification(file: UploadFile = File(...), session: Session = Depends(get_session)):
    image, image_id, file_path = save_image(file)
    image = np.array(image)
    image = cv2.cvtColor(image, cv2.COLOR_GRAY2RGB)
...
def save_image(file: UploadFile):
    ...
    file_path = config.IMAGE_DB_HOME / image_id+".png"
